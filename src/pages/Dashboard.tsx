...
// --- INICIO DE LA MODIFICACIÓN 1: Importar ScrollArea ---
...
export default function Dashboard() {
  ...
  const filteredFoods = useMemo(() => {
    ...
    return MEXICAN_FOODS.filter(food => {
      ...
      const foodCalories = Math.round((food.caloriesPer100g * food.defaultServing) / 100);
  ...
  const totalCalories = useMemo(() => {
    return todayIntake.reduce((sum, item) => sum + (item.baseCalories * item.quantity), 0);
  }, [todayIntake]);
  ...
  const updateAndSaveIntake = (updatedIntake: IntakeItem[]) => {
    const newTotalCalories = updatedIntake.reduce((sum, item) => sum + (item.baseCalories * item.quantity), 0);
  ...
  const handleAddFood = (food: Food) => {
    ...
    if (existingItem) {
      ...
      toast.success(`${food.name} (+1)`);
    } else {
      const calories = Math.round((food.caloriesPer100g * food.defaultServing) / 100);
      const newItem: IntakeItem = {
        foodId: food.id,
        foodName: food.name,
        baseServingSize: food.defaultServing,
        baseCalories: calories,
        quantity: 1,
        timestamp: new Date().toISOString(),
      };
      updateAndSaveIntake([...todayIntake, newItem]);
      toast.success(`${food.name} agregado (${calories} kcal)`);
    }
  };
  ...
  const handleExportPDF = () => {
    ...
    todayIntake.forEach((item, i) => {
      ...
      doc.text(`${i + 1}. ${item.foodName} (x${item.quantity}) - ${totalServing}g - ${totalCalories} kcal`, 25, y);
      y += 10;
    });
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* --- INICIO DE LA MODIFICACIÓN 2: Aplicar ScrollArea a los resultados --- */}
              {/* Le damos una altura fija (h-[600px]) y padding (pr-4) para el scrollbar */}
              <ScrollArea className="h-[725px] w-full pr-4">
                {filteredFoods.length > 0 ? (
                  // Si hay resultados, los mostramos en la cuadrícula
                  ...
                  // Si NO hay resultados, mostramos el mensaje de "no encontrados"
                  <Card className="p-8 text-center border-dashed border-2 border-primary/20 bg-card/80 backdrop-blur-sm">
              ...
              {/* El mensaje de 'no encontrados' de aquí abajo ya no es necesario */}
              {/*
              {filteredFoods.length === 0 && (
                <Card className="p-8 text-center border-dashed border-2 border-primary/20 bg-card/80 backdrop-blur-sm">
                  <p className="text-muted-foreground">
